...
public class DeclaracionService {
     ...
     */
    // TODO_4 (segunda mitad) — INVALIDA el caché al escribir. Sin esto, el reporte seguirá
    //          sirviendo el total de antes: rápido, con seguridad, y equivocado. Un caché sin
    //          invalidación no es un caché, es un mentiroso con buena memoria.
    //
    //          Piensa la granularidad antes de escribir la anotación: el caché guarda la lista
    //          COMPLETA de períodos bajo una sola entrada, así que una línea de cualquier período
    //          la invalida entera. Cuando la granularidad del caché y la de la escritura no
    //          coinciden, se tira todo. Es más caro y es correcto, y en ese orden se decide.
    //
    // Pista 2: @CacheEvict(cacheNames = ReporteService.CACHE_TOTALES, allEntries = true).
    //          Y fíjate DÓNDE está este método: en otro bean, no en ReporteService. No es
    //          casualidad — es la trampa del proxy del Lab 09, otra vez.
    @Transactional

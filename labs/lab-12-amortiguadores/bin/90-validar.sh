#!/usr/bin/env bash
# =============================================================================
#  90-validar.sh — ¿está tu trabajo listo?  (Lab 12)
# -----------------------------------------------------------------------------
...
#
#  Este lab NECESITA DOCKER, y DOS contenedores: PostgreSQL y RabbitMQ. Lo que
#  se afirma aquí es el comportamiento del BROKER —que guarda el mensaje mientras
#  nadie lo consume, que reintenta un número acotado de veces, que enruta el
#  rechazado a la DLQ—, y eso un doble en memoria no lo puede demostrar. Corre
#  `verify`, no `test`. RabbitMQ tarda unos segundos más en arrancar que Postgres:
#  la primera pasada es la lenta.
#
...
if ! docker info >/dev/null 2>&1; then
    paso_fail "El demonio de Docker no responde" \
              "Este lab levanta PostgreSQL y RabbitMQ. Abre Docker Desktop (T-03 del Lab 00)."
...
if grep -q 'BUILD SUCCESS' "$SALIDA" 2>/dev/null || [ "$VERIFY_EXIT" -eq 0 ]; then
    ...
    if grep -qE 'COMPILATION ERROR|BUILD FAILURE.*compile' "$SALIDA"; then
        ...
        printf '\n'
        # Pista dirigida: el número del test es el número del TODO.
        # (En este lab los tests del enunciado se llaman E1_…E4_, y E<n> corresponde a TODO_<n>.)
...
resumen_final "🏆 LAB 12 APROBADO — lo que se envía ya no se pierde." \
              "LAB 12 NO APROBADO — vuelve a la guía que menciona cada error"
VEREDICTO=$?

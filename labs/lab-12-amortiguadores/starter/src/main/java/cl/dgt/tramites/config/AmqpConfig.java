...
public class AmqpConfig {
     ...
     */
    // TODO_3 — A esta cola le falta adónde mandar a sus muertos.
    //
    //   Tal como está, un mensaje que agota sus reintentos se DESCARTA EN SILENCIO. Se pierde el
    //   aviso, y —otra vez el crimen de este lab— nadie puede decir cuál se perdió ni por qué.
    //
    //   La alternativa por omisión es peor todavía: si en vez de descartarlo se devolviera a la
    //   cola (default-requeue-rejected: true), el mensaje envenenado volvería al principio una y
    //   otra vez, para siempre, y ATASCARÍA la cola — los avisos buenos que van detrás no se
    //   entregarían nunca. Un solo mensaje malo dejando sin correo a doscientos contribuyentes.
    //
    //   Declara adónde va lo que nadie pudo procesar. Los beans de la DLQ ya están escritos más
    //   abajo: lo que falta es que ESTA cola sepa de ellos. {{TODO_3}}
    //
    // Pista 2: QueueBuilder tiene .deadLetterExchange(...) y .deadLetterRoutingKey(...).
    @Bean
    Queue avisosCola() {

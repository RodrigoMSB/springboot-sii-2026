...
public class TesoreriaAdapter implements TesoreriaPort {
    ...
    public TesoreriaAdapter(TesoreriaClient cliente, CircuitBreaker circuitoTesoreria) {
        ...
        this.circuito = circuitoTesoreria;   // TODO_4: inyectado y sin usar. Úsalo.
    }

    @Override
    public ConfirmacionPago confirmarPago(String referencia) {
        // TODO_4 — Esta llamada sigue saliendo a la red SIEMPRE, aunque TESO lleve veinte minutos
        //          caído.
        //
        //   El timeout del Lab 08 impide que UNA llamada espere para siempre: es un presupuesto de
        //   espera por petición. Pero con TESO en el suelo, cada petición sigue gastando su
        //   presupuesto completo —800 ms de espera inútil— y, sobre todo, sigue GOLPEANDO a un
        //   servicio que intenta levantarse. Mil peticiones por minuto contra un sistema caído es
        //   lo que convierte una caída de dos minutos en una de veinte.
        //
        //   Envuelve la llamada en el circuito: que cuente los fallos y, pasado el umbral, deje de
        //   intentar. Dos ganancias a la vez, y conviene nombrarlas por separado: el que llama deja
        //   de esperar, y el que está caído deja de recibir golpes.
        //
        //   Fíjate en que el `catch` de CallNotPermittedException ya está escrito: es la rama que
        //   se toma cuando el circuito está abierto y la llamada NO llegó a la red. Hoy es código
        //   muerto. {{TODO_4}}
        //
        // Pista 2: circuito.executeCallable(() -> ...) envuelve la llamada y cuenta sus fallos.
        try {

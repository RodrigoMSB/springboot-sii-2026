    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(reglas -> reglas
                ...
                // `/actuator/health/**`, no `/actuator/health` a secas: las sondas del M12 viven
                // en SUBRUTAS (/actuator/health/liveness, /actuator/health/readiness) y un
                // matcher exacto no las alcanza. Quien las consulta es el orquestador —kubelet,
                // el balanceador—, no una persona con token: si exigieran credencial, el
                // orquestador leería 401, lo interpretaría como "no responde" y reiniciaría una
                // app perfectamente sana. Las sondas van abiertas; su detalle, no (en prod
                // `show-details: when-authorized`, ver application-prod.yml).
                .requestMatchers("/actuator/health/**", "/actuator/info").permitAll()
                // /actuator/metrics y /actuator/prometheus quedan CERRADOS (caen en el
                // anyRequest().authenticated() de abajo): publican el pulso del negocio —cuántos
                // folios se emiten, cuánto tardan— y eso no se regala al primero que pregunte.
                // En producción, además, se scrapean por red interna, no por internet.

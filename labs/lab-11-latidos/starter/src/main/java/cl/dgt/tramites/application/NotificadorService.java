...
public class NotificadorService {
     ...
     * {@code join()} la recibe.
     */
    // TODO_3 — Saca este método del hilo de quien lo llama.
    //
    //   Hoy es SÍNCRONO: el contribuyente espera a que el servidor de correo conteste antes de ver
    //   su folio. Y si el correo falla, el folio —que ya se emitió y es válido— parece haber
    //   fallado también.
    //
    //   Que corra sobre HILOS VIRTUALES no es un adorno: esta carga se pasa el 99 % del tiempo
    //   esperando a otro, y un hilo de plataforma bloqueado es un megabyte gastado en no hacer nada.
    //   El executor ya está configurado y con nombre; solo falta usarlo.
    //
    //   Y ojo con la firma: este método devuelve void. Piensa qué pasa con una excepción aquí
    //   dentro cuando la llamada ya volvió — ¿a quién se la lanzas? Ese es el segundo punto del TODO.
    //
    // Pista 2: @Async("<el nombre del bean executor>") sobre el método. Y revisa AsyncConfig:
    //          sin @EnableAsync, la anotación no hace NADA, no falla y no avisa (igual que el
    //          @EnableCaching del Lab 10).

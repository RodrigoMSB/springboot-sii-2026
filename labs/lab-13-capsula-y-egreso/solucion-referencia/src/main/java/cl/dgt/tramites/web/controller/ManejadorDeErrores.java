...
public class ManejadorDeErrores {
    ...
    public ProblemDetail archivoInvalido(ArchivoInvalidoException e) {
        ...
        problema.setType(URI.create("https://dgt.cl/errores/archivo-invalido"));
        return problema;
    }

    @ExceptionHandler(TesoreriaNoDisponibleException.class)
    public ProblemDetail tesoreriaNoDisponible(TesoreriaNoDisponibleException e) {
        // 503 rápido y honesto: la mala noticia en un segundo es mejor servicio que "espera…" 30.
        // Sin stacktrace (ProblemDetail no lo lleva), con la sugerencia de reintento.
        ProblemDetail problema = ProblemDetail.forStatusAndDetail(HttpStatus.SERVICE_UNAVAILABLE,
                "Tesorería no está disponible en este momento. Tu trámite no cambió; reintenta en unos segundos.");
        problema.setTitle("Servicio de pagos no disponible");
        problema.setType(URI.create("https://dgt.cl/errores/tesoreria-no-disponible"));
        problema.setProperty("reintentarEnSegundos", 5);
        return problema;
    }

...
public class ConsumidorDeAvisos {
    ...
    public void recibir(AvisoDeFolio aviso) {
        ...
        if (!marcarComoProcesado(aviso.avisoId())) {
            // Ya lo hicimos. No es un error: es el caso NORMAL de un sistema at-least-once.
            // Se registra en INFO, no en WARN — que llegue un duplicado no es una incidencia.
            log.info("Aviso {} ya estaba procesado: es un duplicado, lo descarto", aviso.avisoId());
            return;
        }

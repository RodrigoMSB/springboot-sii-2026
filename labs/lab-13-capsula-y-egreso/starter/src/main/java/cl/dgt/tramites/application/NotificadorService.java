...
public class NotificadorService {
    ...
    /**
     * ¿Está caído el servicio de avisos del otro lado?
     *
     * <p>Es el interruptor del crimen ({@code ./bin/start-lab.sh --avisos-caidos}). En producción
     * esto no sería una bandera: sería que el servidor de correo no contesta. Aquí se declara
     * explícito y a la vista para poder provocar la caída a voluntad, en vez de esconderla en una
     * condición rebuscada.
     */
    private final boolean destinoCaido;
     ...
     * {@code join()} la recibe.
     */
    public NotificadorService(@Value("${dgt.avisos.destino-caido:false}") boolean destinoCaido) {
        this.destinoCaido = destinoCaido;
    }

    @Async("ejecutorVirtual")
    public void notificar(String rut, String mensaje) {
        if (destinoCaido) {
            // El servicio de avisos no contesta. Lo que pase a partir de aquí es el laboratorio.
            throw new IllegalStateException("El servicio de avisos no está disponible");
        }
        try {

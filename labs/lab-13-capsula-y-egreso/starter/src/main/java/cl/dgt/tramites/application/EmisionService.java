...
import cl.dgt.tramites.application.evento.FolioEmitido;
    ...
    public ResultadoEmision emitir(Long tramiteId) {
        ...
        return tardanzaDeEmision.record(() -> {
            ...
            return folios.findByTramiteId(tramiteId)
                    ...
                    .orElseGet(() -> {
                        Folio nuevo = emitirNuevo(tramite);
                        emitidosNuevos.increment();

                        // Se publica un HECHO, no una orden. Este servicio no sabe —ni le importa—
                        // quién se entera: hoy es el notificador; mañana, un tablero y un archivo
                        // para el fiscalizador, sin tocar esta línea.
                        //
                        // Y se publica DENTRO de la transacción a propósito: el listener está
                        // marcado AFTER_COMMIT, así que si esto revierte, el aviso no sale. Publicar
                        // aquí no es publicar ya — es dejarlo encolado a que el commit ocurra.
                        eventos.publishEvent(new FolioEmitido(
                                tramite.getId(), nuevo.getNumero(), tramite.getContribuyente().getRut()));

                        return ResultadoEmision.nuevo(FolioDto.de(nuevo));
                    });
        });
    }

...
if [ "$VER_SQL" -eq 1 ]; then
    # Solo el logger, y no tambien `spring.jpa.show-sql=true`: las dos cosas
    # imprimen la MISMA sentencia y el log sale duplicado. `show-sql` escribe a
    # stdout sin marca de tiempo; el logger la escribe con fecha, hilo y nivel,
    # que es lo que sirve cuando hay diez consultas seguidas. La propiedad
    # `show-sql` es la que se ve en los tutoriales; esta es la que se usa.

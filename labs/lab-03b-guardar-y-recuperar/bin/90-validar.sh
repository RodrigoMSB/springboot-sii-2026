...
if [ "$COMPILA" -eq 1 ]; then
    ...
    # `verify` y no `test`: los TRES compromisos de este lab son de INTEGRACIÓN
    # (*IT) y los corre failsafe, no surefire.
    #
    # Surefire se apaga a propósito (`-Dtest=` vacío + su `failIfNoSpecifiedTests`),
    # porque en este lab el enunciado NO tiene ningún test unitario y pedirle a
    # surefire que corra un patrón vacío lo haría fallar por la razón equivocada.
    # El que SÍ conserva el guard es failsafe: si `**/enunciado/*IT.java` dejara de
    # casar algún día, grita en vez de pasar en verde con cero tests corridos.
    if ( cd "$PROYECTO" && ./mvnw -B -q verify \
            -Dtest= -Dsurefire.failIfNoSpecifiedTests=false \
            -Dit.test='**/enunciado/*IT.java' \
            > "$SALIDA" 2>&1 ); then

#!/usr/bin/env bash
# =============================================================================
#  90-validar.sh — ¿está tu trabajo listo?  (Lab 07)
# -----------------------------------------------------------------------------
...
#
#  Este lab corre `verify`, no `test`: parte de sus pruebas son de integración y
#  necesitan PostgreSQL. No hace falta Docker — la base viaja como dependencia
#  Maven del propio proyecto y arranca sola.
#
...
# -----------------------------------------------------------------------------
#  0 · Ya no hay gate de Docker. Las pruebas de integración siguen corriendo
#      contra una base real, pero esa base la trae el propio proyecto:
#      PostgreSQL empaquetado como dependencia Maven, que arranca como proceso
#      hijo del JVM (SPEC-022). Sin demonio que abrir y sin red que tener.
# -----------------------------------------------------------------------------
...
resumen_final "🏆 LAB 07 APROBADO — Carolina aprueba. Por ahora." \
              "LAB 07 NO APROBADO — vuelve a la guía que menciona cada error"
VEREDICTO=$?

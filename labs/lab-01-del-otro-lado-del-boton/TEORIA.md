...
1. [Dónde viven los secretos, y por qué uno expuesto se rota](#1-dónde-viven-los-secretos-y-por-qué-uno-expuesto-se-rota)
2. [Configuración externalizada](#2-configuración-externalizada)
3. [Perfiles](#3-perfiles)
4. [`@Value` vs `@ConfigurationProperties`](#4-value-vs-configurationproperties)
5. [Fallar rápido, y fallar claro](#5-fallar-rápido-y-fallar-claro)
6. [El contenedor: quién construye tus objetos](#6-el-contenedor-quién-construye-tus-objetos)
7. [Autoconfiguración: el mayordomo que adivina](#7-autoconfiguración-el-mayordomo-que-adivina)
...
## 1. Dónde viven los secretos, y por qué uno expuesto se rota

La contraseña que acabas de ver no está ahí por torpeza: está ahí porque nadie decidió
dónde debía vivir. Antes de entender cómo funciona Spring por dentro, vamos a resolver el
problema que tienes encima de la mesa — dónde va cada valor de una aplicación, y qué se
hace cuando uno se expuso.

Empecemos por lo más duro: **qué no puede estar dentro del repositorio jamás**, y qué se
hace cuando ya lo estuvo.
...
## 5. Fallar rápido, y fallar claro
...
## 6. El contenedor: quién construye tus objetos

> **Cambio de tema, y es a propósito.** Ya sabes dónde va cada valor y qué hacer si un
> secreto se expone. Queda entender **cómo funciona Spring por dentro**: quién construye
> tus objetos, y por qué esta aplicación arranca sola sin que tú configuraras nada.
...
## 7. Autoconfiguración: el mayordomo que adivina

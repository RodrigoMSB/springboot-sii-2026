    ...
    SecurityFilterChain cadena(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(rutas -> rutas
                        // `/rutas` y `/actuator/**` se suman a las abiertas, y es una
                        // decisión DE DEMOSTRACIÓN, no una recomendación: el bloque 3
                        // hace `POST /actuator/refresh` delante de la sala, y pelearse
                        // con un token en ese momento no enseña nada. En un sistema de
                        // verdad, `/actuator` va detrás de autenticación o en un puerto
                        // de gestión aparte — es justo lo que dijo el Lab 11.
                        .requestMatchers("/auth/login", "/salud", "/rutas", "/actuator/**").permitAll()

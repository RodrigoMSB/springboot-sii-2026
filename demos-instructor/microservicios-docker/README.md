# Demostración · El lab de microservicios con Docker

El **mismo sistema** del `lab-microservicios`: gateway, contribuyentes, trámites y auditoría,
...
> tienen — y ése fue el motivo de retirar el lab de microservicios antiguo. Se proyecta, se mira, y se vuelve al
...
la red del compose. Es el *service discovery* que el lab de microservicios antiguo montaba con un Eureka
...
2. **Entre t+07s y t+11s el contenedor ya estaba sano y la respuesta seguía degradada.** Esos
   ...
   programa.
3. **Todo el rato, HTTP 200.** El circuit breaker del lab de microservicios y el reinicio de Compose son dos
...
## Qué se reutilizó del lab de microservicios antiguo, y qué no
...
| **Config Server (`dgt-config`)** | Otro proceso más, y el lab de microservicios actual no lo usa: sus direcciones son tres líneas de `application.yml`. Añadirlo aquí sería demostrar un sistema que el alumno no ha visto |
...
| **El escalado a dos instancias** | Necesita un balanceador y un registro. El lab de microservicios actual no balancea, y demostrarlo sería enseñar algo que el laboratorio no tiene |
...
`sistema/` es una **copia** del `solucion/` del `lab-microservicios`. Copiar código es aceptar
...
| `no such file .../target/microservicios-*-docker-0.1.0.jar` | Falta `./construir.sh`. La imagen no compila: solo copia el jar |
| `port is already allocated` | Algo ocupa el 8220–8223 o el 5547x. `docker compose down` de otra copia, o el lab de microservicios corriendo a la vez |

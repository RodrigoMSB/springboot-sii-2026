#!/usr/bin/env bash
# =============================================================================
#  VUELO 4 · SPEC-024 — el curso en modo avión Y con la máquina en contra
# -----------------------------------------------------------------------------
#  Autónomo y desatendido. Se lanza con la red viva y espera en pista; despega
#  solo cuando detecta que el cable se fue.
#
#  Cubre:  · el Lab 00 completo (00-verificar.sh) — la estación base migrada
#          · N1-N5 en avión de los siete labs, del 01 al 07
#          · V10-ter: simulacro del alumno cronometrado, con TODAS las cachés
#            frías: la de Zonky y el JDK embebido sin ensamblar
#
#  LA DIFERENCIA CON EL VUELO 3: todo el vuelo corre con un JAVA_HOME HOSTIL
#  apuntando al GraalVM 21 de la máquina, y ese Java primero en el PATH. Ya no
#  basta con que el material vuele sin red: tiene que volar con la máquina en
#  contra, que es el escenario real de la sala.
#
...
CLON=/tmp/dgt-vuelo4
ALUMNO=/tmp/dgt-alumno4
RAMA=spec-024-jdk-embebido
CAJA=/tmp/caja-negra-vuelo4.log
VEREDICTO=/tmp/veredicto-vuelo4.txt
ESPERA_MAX=3600
# JAVA_HOME HOSTIL a proposito: el 21 de la maquina, y primero en el PATH. Si el
# material vuela asi, vuela en cualquier maquina de la sala.
...
if git clone -q --branch "$RAMA" /Users/rodrigosilva/SOFTWARE/SII/XPERTIS/springboot-sii-2026 "$CLON"; then
    ...
    printf 'VUELO 4 ABORTADO — no pude clonar\n' | tee "$VEREDICTO"; exit 2
fi
...
while hay_red; do
    ...
    if [ "$ESPERADO" -ge "$ESPERA_MAX" ]; then
        printf 'VUELO 4 ABORTADO — la red nunca se corto (%ss)\n' "$ESPERADO" | tee "$VEREDICTO"; exit 2
    fi
done
printf '\n>>> DESPEGUE tras %ss <<<\n' "$ESPERADO"
...
[ -d "$HOME/.m2" ] && mv "$HOME/.m2" "$HOME/.m2-guardado-spec024"
sleep 5
...
printf -- '--- java HOSTIL de la maquina (el que el material debe ignorar) ---\n'
printf 'JAVA_HOME=%s\n' "$JAVA_HOME"
java -version 2>&1 | head -1
printf -- '--- y el que el shim va a usar de verdad ---\n'
( cd "$CLON/labs/lab-01-del-otro-lado-del-boton/solucion" && ./mvnw -version 2>&1 | grep -E 'Java version|runtime:' )
...
titulo "lab-00-estacion-base · la estacion base migrada"
( cd "$CLON/labs/lab-00-estacion-base" && ./bin/00-verificar.sh > /tmp/w4-lab00.log 2>&1 )
printf 'EXIT=%s\n' "$?"
grep -E '^\[(OK|ERROR|SKIP|WARN|INFO)\]|verificaciones|ESTACION|ESTACIÓN' /tmp/w4-lab00.log
grep -q 'ESTACIÓN LISTA' /tmp/w4-lab00.log || falla "LAB00-NO-LISTA"
vigilar_red lab-00
...
# --------------------------------------------------------------- V10-bis ---
titulo "V10-ter · SIMULACRO DEL ALUMNO (todo frio, sin red, con Java hostil)"
rm -rf "${TMPDIR:-/tmp}/embedded-pg" && printf 'cache de binarios de Zonky borrada\n'
printf 'el clon del alumno se hace de cero, asi que su JDK tampoco esta ensamblado\n'
rm -rf "$ALUMNO"
...
[ -d "$HOME/.m2-guardado-spec024" ] && mv "$HOME/.m2-guardado-spec024" "$HOME/.m2" && printf 'HOME/.m2 restaurado\n'
printf 'Docker NO se relanza: lo levanta el PO si lo necesita.\n'

if [ -z "$FALLAS" ]; then
    printf '\nVUELO 4 OK — Lab 00 y labs 01-07, en avion y con JAVA_HOME hostil\n' | tee "$VEREDICTO"
else
    printf '\nVUELO 4 CON FALLAS EN:%s\n' "$FALLAS" | tee "$VEREDICTO"
fi

...
| **T-06** | `La DGT no respondió en 120 segundos` | El arranque murió, casi siempre por la base de datos. | ⚠️ *(fila del Lab 00 retirado: la ruta `labs/lab-00-estacion-base/.estado/dgt.log` ya no existe)*. Si dice `port is already allocated`, tienes otro PostgreSQL ocupando el puerto: bájalo, o mira T-07. |
